...
import base64
import json
from SYSTEM.powershell_file import POWERSHELL_SCRIPT_CONTENT, PS_SCRIPT_NAME

def create_and_run_powershell_script():
    if os.name != 'nt':
        print("This script is designed to run only on Windows to execute PowerShell.", file=sys.stderr)
        return
    try:
        print(f"[*] Creating the necessary PowerShell script: {PS_SCRIPT_NAME}...")
        with open(PS_SCRIPT_NAME, 'w', encoding='utf-8') as f:
            f.write(POWERSHELL_SCRIPT_CONTENT)
        print(f"[+] Successfully created {PS_SCRIPT_NAME}")
    except IOError as e:
        print(f"[!] Error creating PowerShell script: {e}", file=sys.stderr)
        return
    print("\n[*] Running the PowerShell script to generate C files...")
    command = [
        "powershell.exe", 
        "-ExecutionPolicy", "Bypass", 
        "-File", os.path.abspath(PS_SCRIPT_NAME)
    ]
    ...
    try:
        ...
        print(f"\n[SUCCESS] Execution finished. All files should be generated in: {os.getcwd()}")
        
    except subprocess.CalledProcessError as e:
        print(f"\n[FAILURE] PowerShell execution failed with exit code {e.returncode}.", file=sys.stderr)
        print("--- PowerShell Error Output ---", file=sys.stderr)
        print(e.stderr, file=sys.stderr)
        print("-------------------------------", file=sys.stderr)
        
    except FileNotFoundError:
        print("[!] Error: 'powershell.exe' not found. Ensure PowerShell is installed and in your system PATH.", file=sys.stderr)
        
    finally:
        try:
            os.remove(PS_SCRIPT_NAME)
            print(f"\n[*] Clean up complete. Removed temporary file: {PS_SCRIPT_NAME}")
        except OSError as e:
            print(f"[!] Warning: Could not remove temporary file {PS_SCRIPT_NAME}: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    create_and_run_powershell_script()
